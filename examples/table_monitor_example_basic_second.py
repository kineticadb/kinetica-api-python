...
EXAMPLE_LOGGER = logging.getLogger(
    "table_monitor_example_basic_second.TableMonitorExampleClient")
...
"""
This examples demonstrates another usage pattern for the GPUdbTableMonitorBase
class. In this example, the client class (TableMonitorExampleClient) defines
the callback methods.

The client class has a member of GPUdbTableMonitorBase.Callbacks type and it is
...
2.  monitor = GPUdbTableMonitorBase(db=h_db,
                                    tablename=tablename,
                                    callbacks=client.callbacks)

    # Start the monitor
3.  monitor.start_monitor()

    # insert some data
4.  load_data()

    # delete a few records
5.  delete_records(h_db)

    # Give some time to the monitor to run
6.  time.sleep(10)

    # Finally stop the monitor
7.  monitor.stop_monitor()
