...
class _GPUdbSqlContextFormatter:
    ...
    @classmethod
    def format_sql(cls, name, ctx_list):
        ...
        return "CREATE OR REPLACE CONTEXT {} {}".format(name, sql_context)
    
    @classmethod
    def _quote_sql_obj(cls, obj):
        parts = obj.split(".")
        parts = ['"{}"'.format(p) for p in parts]
        return ".".join(parts)

    @classmethod
    def _quote_text(cls, text):
        if(len(text) == 0):
            return ""
    ...
    @classmethod
    def _format_clause(cls, params):
        lines = []
        for param, val in params.items():
            if len(val) == 0:
                continue
            lines.append("    {} = {}".format(param, val))
        context = ',\n'.join(lines)
        return "(\n{}\n)".format(context)
...
class GPUdbSqlContext:
    ...
    def __repr__(self):
        return "SqlContext(name={})".format(self.name)
